...
class StrategyManager:
    ...
    def select_queries(self, data_module, method_list, module, n_queries):
        ...
        if 'egl' in method_list:
            smax_out = F.softmax(self.logits, dim=-1)
            grad_lens = self.extract_grad_lens(data_module, module, self.criterion)
        ...
        if 'pert' in method_list:
            change_percentages = self.perturbate(data_module, module)
        ...
        if 'cover' in method_list:
            idxs_dict['cover'] = self.cover_distribution(data_module, n_queries)
    ...
    def get_dists(self, samples, keep_same_idx=True, until_idx=None):
        all_dists = []
    ...
    def perturbate(self, data_module, module):
    ...
    def cover_distribution(self, data_module, n_queries):
        # Get distances
        if hasattr(data_module, 'unlabeled_dataloader'):
            loader = data_module.unlabeled_dataloader()
        else:
        ...
        D = self.get_dists(samples, True, len(new_samples))
